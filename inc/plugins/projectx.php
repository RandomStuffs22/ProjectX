...
function projectx_activate()
{
	...
	$PL->settings('projectx',
		$lang->projectx_settings,
		$lang->projectx_settings_desc,
		array(
			'enabled'   =>  array(
...
function projectx_true_install()
{
	...
	$PL->edit_core('ProjectX', 'admin/jscripts/tabs.js',
				array(
				// needed for submenu
					array('search' => 'setClassOnContainer: false,',
                     	  'replace' => 'setClassOnContainer: true,'),
					 ),
               true);
				   
	$PL->edit_core('ProjectX', 'admin/modules/home/module_meta.php',
				array(
				// At the current stages ProjectX isn't capable of displaying correctly online admins. So, just hide variables
					array('search' => '$sidebar = new SidebarItem($lang->online_admins);
	...
	$PL->edit_core('ProjectX', 'admin/modules/config/module_meta.php',
				array(
				// Split the menu into two rows - hide present elements
					array('search' => '$sub_menu[\'110\'] = array("id" => "attachment_types", "title" => $lang->attachment_types, "link" => "index.php?module=config-attachment_types");
	...
	$sub_menu = $plugins->run_hooks("admin_config_menu", $sub_menu);',
				...
				// Split the menu into two rows - make them appear again!
					array('search' => '$actions = $plugins->run_hooks("admin_config_action_handler", $actions);',
	...
	$db->update_query('settings', array('value' => $db->escape_string("ProjectX")), "name = 'cpstyle'");
	$db->update_query('adminoptions', array('cpstyle' => $db->escape_string("ProjectX")));
}
...
function projectx_true_uninstall()
{
	...
	$db->update_query('settings', array('value' => $db->escape_string("default")), "name = 'cpstyle'");
	$db->update_query('adminoptions', array('cpstyle' => $db->escape_string("default")));
